...
import cn.chyuan.ai.domain.admin.adapter.respository.IAdminRepository;
import cn.chyuan.ai.infrastructure.dao.*;
...
import cn.chyuan.ai.infrastructure.dao.po.McpGatewayAuthPO;
...
public class AdminRepository implements IAdminRepository {
        ...
        public List<GatewayToolConfigEntity> queryGatewayToolList() {
                List<cn.chyuan.ai.infrastructure.dao.po.McpGatewayToolPO> mcpGatewayToolPOS = mcpGatewayToolDao
                                .queryAll();
        ...
        public List<GatewayProtocolConfigEntity> queryGatewayProtocolList() {
                ...
                // 批量查询所有协议映射（避免 N+1 查询）
                List<Long> protocolIds = pos.stream()
                                .map(McpProtocolHttpPO::getProtocolId)
                                .distinct()
                                .collect(Collectors.toList());

                List<McpProtocolMappingPO> allMappings = protocolMappingDao.queryListByProtocolIds(protocolIds);
                ...
                return pos.stream().map(po -> {
                        List<McpProtocolMappingPO> mappings = mappingsByProtocolId.getOrDefault(po.getProtocolId(),
                                        new ArrayList<>());
                        return GatewayProtocolConfigEntity.builder()
                                        ...
                                        .mappings(mappings.isEmpty() ? null
                                                        : mappings.stream().map(
                                                                        m -> GatewayProtocolConfigEntity.ProtocolMappingEntity
                                                                                        .builder()
        ...
        public GatewayToolPageEntity queryGatewayToolPage(GatewayToolQueryEntity queryEntity) {
                ...
                List<GatewayToolConfigEntity> dataList = mcpGatewayToolPOS.stream()
                                .map(po -> GatewayToolConfigEntity.builder()
                                                .gatewayId(po.getGatewayId())
                                                .toolId(po.getToolId())
                                                .toolName(po.getToolName())
                                                .toolType(po.getToolType())
        ...
        public GatewayProtocolPageEntity queryGatewayProtocolPage(GatewayProtocolQueryEntity queryEntity) {
                ...
                List<Long> protocolIds = pos.stream().map(McpProtocolHttpPO::getProtocolId)
                                .collect(Collectors.toList());
