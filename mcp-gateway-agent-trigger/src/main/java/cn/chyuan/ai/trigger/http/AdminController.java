...
import cn.chyuan.ai.api.IAdminService;
import cn.chyuan.ai.api.dto.*;
import cn.chyuan.ai.api.response.Response;
import cn.chyuan.ai.api.response.ResponsePage;
import cn.chyuan.ai.cases.admin.*;
import cn.chyuan.ai.domain.admin.model.entity.*;
...
import cn.chyuan.ai.domain.gateway.model.valobj.GatewayConfigVO;
import cn.chyuan.ai.domain.gateway.model.valobj.GatewayToolConfigVO;
import cn.chyuan.ai.domain.protocol.model.entity.StorageCommandEntity;
...
import cn.chyuan.ai.types.enums.GatewayEnum;
...
public class AdminController implements IAdminService {
        ...
        public Response<GatewayConfigResponseDTO> saveGatewayConfig(
                        @Valid @RequestBody GatewayConfigRequestDTO.GatewayConfig requestDTO) {
                try {
                        ...
                        GatewayConfigCommandEntity commandEntity = GatewayConfigCommandEntity.builder()
                                        .gatewayConfigVO(GatewayConfigVO.builder()
                                                        ...
                                                        .auth(GatewayEnum.GatewayAuthStatusEnum
                                                                        .getByCode(requestDTO.getAuth()))
        ...
        @Override
        public Response<GatewayConfigResponseDTO> saveGatewayToolConfig(
                        @Valid @RequestBody GatewayConfigRequestDTO.GatewayToolConfig requestDTO) {
                try {
        ...
        @Override
        public Response<GatewayConfigResponseDTO> saveGatewayProtocol(
                        @Valid @RequestBody GatewayConfigRequestDTO.GatewayProtocol requestDTO) {
                try {
                        log.info("保存网关协议配置开始");
                        ...
                        if (requestDTO.getHttpProtocols() != null) {
                                commandEntity.setHttpProtocolVOS(requestDTO.getHttpProtocols().stream().map(p -> {
                                        ...
                                        if (p.getMappings() != null) {
                                                vo.setMappings(p.getMappings().stream()
                                                                .map(m -> HTTPProtocolVO.ProtocolMapping.builder()
        ...
        @Override
        public Response<GatewayConfigResponseDTO> importGatewayProtocol(
                        @Valid @RequestBody GatewayConfigRequestDTO.GatewayProtocolImport requestDTO) {
                try {
                        log.info("导入网关协议配置开始");
        ...
        @Override
        public Response<List<GatewayProtocolDTO>> analysisProtocol(
                        @Valid @RequestBody GatewayConfigRequestDTO.GatewayProtocolImport requestDTO) {
                try {
                        log.info("解析网关协议配置开始");
                        AnalysisCommandEntity commandEntity = AnalysisCommandEntity.builder()
                                        ...
                                        .endpoints(requestDTO.getEndpoints())
                                        .build();
                        List<cn.chyuan.ai.domain.protocol.model.valobj.http.HTTPProtocolVO> httpProtocolVOS = adminProtocolService
                                        .analysisProtocol(commandEntity);

                        List<GatewayProtocolDTO> dtoList = httpProtocolVOS.stream().map(e -> GatewayProtocolDTO
                                        .builder()
                                        .httpUrl(e.getHttpUrl())
                                        .httpMethod(e.getHttpMethod())
                                        .httpHeaders(e.getHttpHeaders())
                                        .timeout(e.getTimeout())
                                        .mappings(e.getMappings() == null ? null
                                                        : e.getMappings().stream()
                                                                        .map(m -> GatewayProtocolDTO.ProtocolMappingDTO
                                                                                        .builder()
                                                                                        .mappingType(m.getMappingType())
                                                                                        .parentPath(m.getParentPath())
                                                                                        .fieldName(m.getFieldName())
        ...
        @Override
        public Response<GatewayConfigResponseDTO> saveGatewayAuth(
                        @Valid @RequestBody GatewayConfigRequestDTO.GatewayAuth requestDTO) {
                try {
        ...
        public ResponsePage<List<GatewayConfigDTO>> queryGatewayConfigPage(GatewayConfigQueryDTO queryDTO) {
                try {
                        log.info("查询网关配置分页开始 gatewayId: {}, gatewayName: {}, page: {}, rows: {}",
                                        queryDTO.getGatewayId(), queryDTO.getGatewayName(), queryDTO.getPage(),
                                        queryDTO.getRows());
                        ...
                        List<GatewayConfigDTO> dtoList = pageEntity.getDataList().stream()
                                        .map(e -> GatewayConfigDTO.builder()
                                                        .gatewayId(e.getGatewayId())
                                                        .gatewayName(e.getGatewayName())
                                                        .gatewayDesc(e.getGatewayDesc())
                                                        .version(e.getVersion())
        ...
        public ResponsePage<List<GatewayToolConfigDTO>> queryGatewayToolPage(GatewayToolQueryDTO queryDTO) {
                try {
                        log.info("查询网关工具分页开始 gatewayId: {}, toolId: {}, page: {}, rows: {}",
                                        queryDTO.getGatewayId(), queryDTO.getToolId(), queryDTO.getPage(),
                                        queryDTO.getRows());
                        ...
                        List<GatewayToolConfigDTO> dtoList = pageEntity.getDataList().stream()
                                        .map(e -> GatewayToolConfigDTO.builder()
                                                        .gatewayId(e.getGatewayId())
                                                        .toolId(e.getToolId())
                                                        .toolName(e.getToolName())
                                                        .toolType(e.getToolType())
        ...
        public Response<List<GatewayToolConfigDTO>> queryGatewayToolListByGatewayId(@RequestParam String gatewayId) {
                try {
                        ...
                        List<GatewayToolConfigEntity> entities = adminManageService
                                        .queryGatewayToolListByGatewayId(gatewayId);
                        List<GatewayToolConfigDTO> dtoList = entities.stream().map(e -> GatewayToolConfigDTO.builder()
                                        .gatewayId(e.getGatewayId())
                                        .toolId(e.getToolId())
                                        .toolName(e.getToolName())
        ...
        @Override
        public Response<GatewayConfigResponseDTO> deleteGatewayToolConfig(@RequestParam String gatewayId,
        ...
        public ResponsePage<List<GatewayProtocolDTO>> queryGatewayProtocolPage(GatewayProtocolQueryDTO queryDTO) {
                try {
                        log.info("查询网关协议分页开始 protocolId: {}, httpUrl: {}, page: {}, rows: {}",
                                        queryDTO.getProtocolId(), queryDTO.getHttpUrl(), queryDTO.getPage(),
                                        queryDTO.getRows());
                        ...
                        List<GatewayProtocolDTO> dtoList = pageEntity.getDataList().stream().map(e -> GatewayProtocolDTO
                                        .builder()
                                        .protocolId(e.getProtocolId())
                                        .httpUrl(e.getHttpUrl())
                                        .httpMethod(e.getHttpMethod())
        ...
        public Response<List<GatewayProtocolDTO>> queryGatewayProtocolListByGatewayId(@RequestParam String gatewayId) {
                try {
                        ...
                        List<GatewayProtocolConfigEntity> entities = adminManageService
                                        .queryGatewayProtocolListByGatewayId(gatewayId);
                        List<GatewayProtocolDTO> dtoList = entities.stream().map(e -> GatewayProtocolDTO.builder()
                                        .protocolId(e.getProtocolId())
                                        .httpUrl(e.getHttpUrl())
                                        .httpMethod(e.getHttpMethod())
        ...
        public Response<List<GatewayAuthDTO>> queryGatewayAuthListByGatewayId(@RequestParam String gatewayId) {
                try {
                        ...
                        List<GatewayAuthConfigEntity> entities = adminManageService
                                        .queryGatewayAuthListByGatewayId(gatewayId);
                        List<GatewayAuthDTO> dtoList = entities.stream().map(e -> GatewayAuthDTO.builder()
                                        .gatewayId(e.getGatewayId())
                                        .apiKey(e.getApiKey())
                                        .rateLimit(e.getRateLimit())
        ...
        @Override
        public Response<GatewayLLMResponseDTO> testCallGateway(@Valid @RequestBody GatewayLLMRequestDTO requestDTO) {
                try {
                        // 对 API Key 进行脱敏处理，只显示前6位和后4位
                        String maskedApiKey = maskApiKey(requestDTO.getAuthApiKey());
                        log.info("测试请求网关服务开始 gatewayId: {} authApiKey: {} reqDTO:{}", requestDTO.getGatewayId(),
                                        maskedApiKey, JSON.toJSONString(requestDTO));

                        GatewayLLMResponseDTO responseDTO = adminLLMService.testCallGateway(requestDTO);
                        log.info("测试请求网关服务完成 gatewayId: {} authApiKey: {} resDTO:{}", requestDTO.getGatewayId(),
                                        maskedApiKey, JSON.toJSONString(responseDTO));
